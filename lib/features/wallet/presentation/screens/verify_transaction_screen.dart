...
class VerifyTransactionScreen extends StatefulWidget {
  ...
  const VerifyTransactionScreen({super.key, required this.data});

  @override
  State<VerifyTransactionScreen> createState() => _VerifyTransactionScreenState();
}
...
class _VerifyTransactionScreenState extends State<VerifyTransactionScreen> {
  ...
  Future<void> _requestOtp() async {
    ...
    setState(() { _isRequestingOtp = true; _error = null; });
    try {
  ...
  Future<void> _verify() async {
    ...
    try {
      ...
      if (mounted) {
        context.pushReplacement('/transaction/success', extra: {
          ...widget.data,
      ...
      // Timeout: idempotency key ensures it was accepted — go to success
  ...
  String get _typeLabel {
    switch (widget.data['type'] as String) {
      case 'deposit':  return 'Authorize deposit';
      case 'withdraw': return 'Authorize withdrawal';
      default:         return 'Authorize transfer';
    }
  }
                ...
                SizedBox(
                  ...
                  child: ElevatedButton(
                    ...
                    style: ElevatedButton.styleFrom(
                      ...
                      disabledBackgroundColor: AppColors.primaryNavy.withOpacity(0.45),
        ...
        children: [
          Row(
            ...
            children: List.generate(6, (i) {
              final isFocused = _focusNode.hasFocus && _code.length == i ||
  ...
  Widget _buildCountdown() {
    final bool canResend = _countdown == 0 && !_isRequestingOtp && _otpEverRequested;
    if (_isRequestingOtp) {
      ...
      child: Column(
        children: [
          ...
          Text(
            'Verification locked',
            style: GoogleFonts.dmSans(
              fontSize: 16, fontWeight: FontWeight.bold, color: AppColors.error),
          ),
          const SizedBox(height: 8),
          Text(
            'You have exceeded the maximum number of verification attempts. Please try again later or contact support.',
            textAlign: TextAlign.center,
            style: GoogleFonts.dmSans(fontSize: 13, color: AppColors.textSecondary, height: 1.55),
          ),
        ],
      ),
    );
